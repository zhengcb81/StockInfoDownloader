...
class CoverageMonitor:
        ...
        try:
            ...
            for filename in cov.get_data().measured_files():
                if source_dir in filename:
                    ...
                    total_lines += len(analysis[1]) + len(analysis[2]) + len(analysis[3])
            ...
            logger.info(f"覆盖率收集完成: {coverage_percentage:.2f}% ({covered_lines}/{total_lines})")
            return metrics
    ...
    def export_report(self, output_file: str = None) -> str:
        ...
        if output_file is None:
            output_file = self.coverage_data_dir / f"coverage_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
