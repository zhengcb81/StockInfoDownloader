...
class PerformanceMetrics:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            ...
            and self.memory_usage >= 0
            and self.cpu_usage >= 0
            and self.throughput >= 0
            and 0 <= self.error_rate <= 1
        )
...
class PerformanceMonitor:
    ...
    def _update_baseline(self, test_name: str):
        ...
        baseline = PerformanceBaseline(
            ...
            created_at=(
                ...
                else self._baselines[test_name].created_at
            ),
    ...
    def check_performance(
        self, test_name: str, metrics: PerformanceMetrics
    ) -> Dict[str, Any]:
        """检查性能是否符合基准"""
        if test_name not in self._baselines:
            return {
        ...
        status = self._determine_performance_status(
            execution_time_deviation, memory_usage_deviation, cpu_usage_deviation
        )
        ...
        performance_check = {
            ...
            "within_thresholds": {
                ...
                <= baseline.execution_time_threshold,
                "memory_usage": metrics.memory_usage <= baseline.memory_usage_threshold,
                "cpu_usage": metrics.cpu_usage
                <= (baseline.cpu_usage * self.config["cpu_threshold_factor"]),
            },
        }
    ...
    def _determine_performance_status(
        self, time_deviation: float, memory_deviation: float, cpu_deviation: float
    ) -> PerformanceStatus:
        """确定性能状态"""
        # 检查是否所有指标都在优秀范围内
        ...
        # 检查是否在可接受范围内
            ...
            time_deviation <= self.config["performance_threshold_factor"]
            and memory_deviation <= self.config["memory_threshold_factor"]
            and cpu_deviation <= self.config["cpu_threshold_factor"]
        ):
        ...
        # 检查是否性能下降
            ...
            time_deviation > self.config["performance_threshold_factor"]
            or memory_deviation > self.config["memory_threshold_factor"]
            or cpu_deviation > self.config["cpu_threshold_factor"]
        ):
    ...
    def _update_stats(self):
        ...
        for test_name, metrics_list in self._performance_data.items():
            ...
            if len(self.stats["worst_performing_tests"]) < 5:
                self.stats["worst_performing_tests"].append(
                    {"test_name": test_name, "average_time": avg_time}
                )
            else:
                # 替换最差的测试
                worst_tests = sorted(
        ...
        # 按执行时间排序
        self.stats["worst_performing_tests"].sort(
    ...
    def get_performance_history(
        ...
        return [
            m for m in self._performance_data[test_name] if m.timestamp >= cutoff_time
        ]
        ...
        report = {
            ...
            "baselines_summary": {
                ...
                "recently_updated": [
                    ...
                    if time.time() - baseline.updated_at
                    < 24 * 60 * 60  # 24小时内更新的
                ],
            },
...
def record_test_performance(
    test_name: str, execution_time: float, **kwargs
) -> PerformanceMetrics:
    """记录测试性能（便捷函数）"""
...
def check_test_performance(
    test_name: str, metrics: PerformanceMetrics
) -> Dict[str, Any]:
    """检查测试性能（便捷函数）"""
    monitor = get_global_performance_monitor()
...
if __name__ == "__main__":
    ...
    metrics1 = monitor.record_performance(
        "test_example_1", execution_time=2.5, memory_usage=50.2, cpu_usage=15.3
    )

    metrics2 = monitor.record_performance(
        "test_example_2", execution_time=1.8, memory_usage=45.1, cpu_usage=12.7
    )
