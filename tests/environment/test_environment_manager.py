...
class TestEnvironmentManager:
    ...
    def __init__(self):
        self.logger = get_logger("test_environment_manager")
    ...
    def check_dependencies(self) -> EnvironmentCheck:
        ...
        try:
            # 检查关键依赖项
            required_packages = [
                "pytest", "playwright", "selenium", "requests",
    ...
    def check_browser_environment(self) -> EnvironmentCheck:
        """检查浏览器环境"""
        component = "浏览器环境"
        log(f"检查 {component}...")
        ...
        try:
            # 检查Playwright浏览器
            playwright_status = "未知"
            selenium_status = "未知"

            try:
                from playwright.sync_api import sync_playwright
                playwright_status = "可用"
            except Exception:
                playwright_status = "不可用"
            ...
            check_passed = playwright_status == "可用" or selenium_status == "可用"
            status = f"Playwright: {playwright_status}, Selenium: {selenium_status}"

            result = EnvironmentCheck(
                component=component,
                status=status,
                version=f"Playwright:{playwright_status},Selenium:{selenium_status}",
                required_version="至少一个可用",
                check_passed=check_passed
            )
            ...
            if check_passed:
                ...
                log(f"❌ {component} 检查失败: 无可用浏览器驱动")
                result.error_message = "无可用浏览器驱动"
    ...
    def check_network_connectivity(self) -> EnvironmentCheck:
        """检查网络连接"""
        component = "网络连接"
        log(f"检查 {component}...")
        ...
        try:
            ...
            check_passed = len(accessible_urls) > 0
            status = f"可访问 {len(accessible_urls)}/{len(test_urls)} 个测试URL"

            result = EnvironmentCheck(
                component=component,
                status=status,
                version=f"{len(accessible_urls)}/{len(test_urls)}",
                required_version="至少一个可访问",
                check_passed=check_passed
            )
            ...
            if check_passed:
                ...
                log(f"❌ {component} 检查失败: 无网络连接")
                result.error_message = "无网络连接"
...
"""
    ...
    log(f"✅ 环境设置脚本已创建: {setup_script_path}")
...
def main():
    ...
    # 创建环境设置脚本
    setup_script_path = create_environment_setup_script()
...
def create_environment_report(result_data: Dict[str, Any], report_file: str):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write("\n## 环境设置脚本\n\n")
        f.write(f"已创建环境设置脚本: `{result_data['setup_script']}`\n")
        f.write("运行此脚本可以自动设置和修复测试环境。\n\n")
