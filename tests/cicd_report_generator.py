...
class CICDReportGenerator(TestReportGenerator):
    ...
    def create_comprehensive_report(self, reports: List[str], description: str = "综合测试报告") -> str:
        """创建综合测试报告"""
        ...
        for report_file in reports:
            try:
                ...
                report_summary = {
                    ...
                    "test_type": report_content.get("test_info", {}).get("test_type", "Unknown"),
        ...
        report_data["combined_summary"]["overall_success_rate"] = (total_passed / total_tests * 100) if total_tests > 0 else 0
        
        # 保存综合报告
        report_file = self.base_dir / "comprehensive" / f"comprehensive_report_{timestamp}.json"
        with open(report_file, 'w', encoding='utf-8') as f:
            json.dump(report_data, f, ensure_ascii=False, indent=2)
        
        return str(report_file)
    
    def create_release_report(self, test_results: str, coverage_data: str, security_data: str, 
                             quality_data: str, description: str = "发布测试报告") -> str:
        """创建发布测试报告"""
        timestamp = self.get_timestamp()
        report_data = {
            "release_info": {
    ...
    def generate_test_summary(self, workflow_name: str, run_id: str, run_number: str) -> str:
        """生成测试汇总报告（HTML格式）"""
        timestamp = self.get_timestamp()
        ...
        try:
            ...
            for counter in root.findall(".//counter"):
                if counter.get("type") == "LINE":
                    ...
                    coverage_data["total_lines"] = int(counter.get("missed", 0)) + coverage_data["covered_lines"]
                    coverage_data["line_coverage"] = coverage_data["covered_lines"] / coverage_data["total_lines"] * 100 if coverage_data["total_lines"] > 0 else 0
