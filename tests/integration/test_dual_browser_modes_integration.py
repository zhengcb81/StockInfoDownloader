...
from tests.utils.test_config_manager import ConfigManagerTool

# 测试股票数据
test_stock = {"code": "300470", "name": "中密控股", "org_id": "gssz0000470"}


@pytest.mark.integration
class TestSeleniumModeIntegration:
    ...
    def setup_method(self):
        ...
        test_mapping = {
            stock.get("code"): {"orgId": stock.get("org_id"), "name": stock.get("name")}
            for stock in test_stocks
        }
        # 添加额外的测试数据
        test_mapping["000001"] = {"orgId": "9900000001", "name": "平安银行"}
            ...
            json.dump(
                {
                    ...
                    "headless": True,
                    "max_retries": 2,
                    "browser": {"strategy": self.browser_strategy, "headless": True},
                    "pages": [
                        {
                            "name": "投资者关系活动",
                            "suffix": "research",
    ...
    def test_download_workflow_consistency(self, mock_get_org_id, mock_create_strategy):
        ...
        download_records = downloader.download_stock_pdfs(
            stock_code=test_stock.get("code", "300470"),
            target_pages=[
                {"suffix": "research", "allowed_keywords": ["投资者关系", "调研"]}
            ],
            max_retries=1,
        )

        # 验证下载结果一致性
        assert isinstance(download_records, dict)
        assert "success" in download_records
    ...
    def test_error_handling_consistency(self, mock_get_org_id, mock_create_strategy):
        ...
        assert "Navigation failed" in error_msg or "导航" in error_msg
    ...
    def test_performance_monitoring_integration(
        self, mock_get_org_id, mock_create_strategy
    ):
        """测试性能监控集成"""
        # 设置mock策略
        mock_strategy = MagicMock()
...
@pytest.mark.integration
class TestPlaywrightModeIntegration:
