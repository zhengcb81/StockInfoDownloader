...
class PerformanceTestRunner:
    ...
    def test_cpu_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            PerformanceAssertions.assert_cpu_usage(
                cpu_usage, 80.0, f"{test_name}CPU使用率过高"  # 最大CPU使用率80%
            )
            PerformanceAssertions.assert_response_time(
                cpu_execution_time, 5.0, f"{test_name}执行时间过长"  # 最大执行时间5秒
            )
    ...
    def test_keyword_matching_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 测试关键词匹配器性能
            config = KeywordConfig(allowed_keywords=keywords, mode="any")
            ...
            # 性能断言
            PerformanceAssertions.assert_response_time(
                matching_time, 1.0, f"{test_name}匹配时间过长"  # 最大匹配时间1秒
            )
            PerformanceAssertions.assert_response_time(
            ...
            return {
                ...
                "metrics": {
                    ...
                    "average_time_per_text_ms": round(
                        matching_time / len(test_texts) * 1000, 3
                    ),
    ...
    def test_file_operations_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 性能断言
            PerformanceAssertions.assert_response_time(
                write_time, 5.0, f"{test_name}文件写入时间过长"  # 最大写入时间5秒
            )
            PerformanceAssertions.assert_response_time(
                read_time, 2.0, f"{test_name}文件读取时间过长"  # 最大读取时间2秒
            )
            ...
            return {
                ...
                "metrics": {
                    ...
                    "average_write_time_per_file_ms": round(
                        write_time / files_created * 1000, 3
                    ),
                    "average_read_time_per_file_ms": round(
                        read_time / files_read * 1000, 3
                    ),
                },
                "performance_criteria": {
    ...
    def test_mapping_operations_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 性能断言
            PerformanceAssertions.assert_response_time(
                load_time, 2.0, f"{test_name}映射加载时间过长"  # 最大加载时间2秒
            )
            PerformanceAssertions.assert_response_time(
                query_time, 1.0, f"{test_name}映射查询时间过长"  # 最大查询时间1秒
            )
            PerformanceAssertions.assert_response_time(
    ...
    def _generate_performance_report(self, results: Dict[str, Any]) -> str:
        ...
        results["summary"] = {
            ...
            "total_execution_time": round(
                sum(test.get("execution_time", 0) for test in results["tests"]), 2
            ),
        }
