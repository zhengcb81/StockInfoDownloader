...
class TestAnalysisOptimizer:
    ...
    def analyze_test_structure(self) -> TestAnalysis:
        ...
        for test_type, test_dir in test_directories:
            ...
            if test_path.exists():
                ...
                log(
                    f"  {test_type}: {test_count} 个测试，预估执行时间: {total_time:.1f}秒"
                )
    ...
    def generate_optimization_plan(
        self, test_analyses: List[TestAnalysis]
    ...
    def calculate_optimization_impact(
        ...
        total_current_time = sum(
            analysis.total_execution_time for analysis in test_analyses
        )
        ...
        for recommendation in recommendations:
            ...
            for analysis in test_analyses:
                if (
                    recommendation.area in analysis.test_type
                    or recommendation.area == "测试执行"
                ):
        ...
        overall_improvement_percentage = (
            (total_improvement / total_current_time) * 100
            if total_current_time > 0
            else 0
        )
...
def main():
    ...
    # 计算优化影响
    impact_analysis = optimizer.calculate_optimization_impact(
        test_analyses, recommendations
    )
...
def create_optimization_report(result_data: Dict[str, Any], report_file: str):
    ...
    with open(report_file, "w", encoding="utf-8") as f:
        ...
        for analysis in result_data["test_analyses"]:
            f.write(
                f"| {analysis['test_type']} | {analysis['test_count']} | {analysis['total_execution_time']:.1f}秒 | {analysis['optimization_potential'] * 100:.0f}% |\n"
            )
        ...
        for i, recommendation in enumerate(result_data["recommendations"], 1):
            ...
            f.write(
                f"- **预期改进**: {recommendation['expected_improvement'] * 100:.0f}%\n"
            )
            f.write(
                f"- **实现难度**: {recommendation['implementation_difficulty']}\n\n"
            )
        ...
        f.write(
            f"- **总体改进百分比**: {impact['overall_improvement_percentage']:.1f}%\n"
        )
