...
class PerformanceAssertions:
    ...
    def assert_response_time(
        actual_time: float, max_time: float, message: str = ""
    ) -> None:
        """
        断言响应时间不超过阈值
    ...
    def assert_success_rate(
        success_count: int, total_count: int, min_rate: float, message: str = ""
    ) -> None:
        """
        断言成功率不低于阈值
        ...
        assert rate >= min_rate, f"成功率不足: {rate:.1f}% < {min_rate:.1f}% {message}"

    @staticmethod
    def assert_memory_growth(
    ...
    def assert_cpu_usage(
        cpu_usage_percent: float, max_usage_percent: float, message: str = ""
    ) -> None:
        """
        断言CPU使用率不超过阈值
        ...
        """
        assert (
            cpu_usage_percent <= max_usage_percent
        ), f"CPU使用率过高: {cpu_usage_percent:.1f}% > {max_usage_percent:.1f}% {message}"

    @staticmethod
    def assert_concurrent_time(
        actual_time: float, max_time: float, message: str = ""
    ) -> None:
        """
        断言并发执行时间不超过阈值
    ...
    def assert_latency_percentile(
        latencies: List[float], percentile: float, max_latency: float, message: str = ""
    ) -> None:
        """
        断言延迟百分位数不超过阈值
    ...
    def assert_cache_hit_rate(
        hit_count: int, total_requests: int, min_hit_rate: float, message: str = ""
    ) -> None:
        """
        断言缓存命中率不低于阈值
    ...
    @staticmethod
    def assert_against_thresholds(
        results: Dict[str, Any], thresholds: PerformanceThreshold, test_name: str = ""
    ) -> None:
        """
        根据阈值配置断言所有性能指标
        ...
        # 检查内存增长
        if "initial_memory_mb" in results and "peak_memory_mb" in results:
        ...
        # 检查并发时间
        if (
            "concurrent_time_seconds" in results
            and thresholds.max_concurrent_time is not None
        ):
        ...
        # 检查吞吐量
        if (
            "tasks_completed" in results
            and "total_time" in results
            and thresholds.min_throughput is not None
        ):
