...
def main():
    parser = argparse.ArgumentParser(
        description="Unified Test Runner for StockInfoDownloader"
    )
    parser.add_argument(
        "type",
        choices=["unit", "integration", "e2e", "performance", "validation", "all"],
        default="all",
        nargs="?",
    ...
    if args.type == "validation":
        # Validation tests might not use markers, run specific directory
        pytest_args.append("tests/validation/")
    elif args.type != "all":
    ...
    if args.html:
        ...
        pytest_args.extend(
            [f"--html=test_reports/report_{args.type}.html", "--self-contained-html"]
        )
