...
class DataManagerTool:
    ...
    def __init__(self, base_test_dir: Optional[str] = None):
        ...
        """
        self.base_test_dir = (
            Path(base_test_dir) if base_test_dir else Path(tempfile.mkdtemp())
        )
    ...
    def simulate_download_with_extra_files(
        self, stock_code: str, extra_files: List[str]
    ) -> List[str]:
        """模拟下载包含额外文件"""
        stock_data = self.get_stock_data(stock_code)
        if not stock_data:
    ...
    def validate_scenario(self, scenario_name: str) -> ValidationResult:
        ...
        return self.validator.validate_expected_data(
            Path(scenario.expected_data_dir), Path(scenario.actual_data_dir)
        )

    def validate_single_stock(
        self, stock_code: str, comparison_mode: ComparisonMode = ComparisonMode.STRICT
    ) -> ValidationResult:
        """验证单个股票"""
        stock_data = self.get_stock_data(stock_code)
        if not stock_data:
        ...
        return self.validator.validate_single_test_case(
            test_case, Path(self.expected_data_dir), Path(self.actual_data_dir)
        )
    ...
    def export_test_data(self, export_dir: str) -> str:
        ...
        if self.expected_data_dir.exists():
            shutil.copytree(
                self.expected_data_dir, expected_export_dir, dirs_exist_ok=True
            )
