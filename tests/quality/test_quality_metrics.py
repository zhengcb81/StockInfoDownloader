...
class TestQualityMetrics:
    ...
    def __init__(self):
        self.logger = get_logger("test_quality_metrics")

    def calculate_test_coverage(self) -> MetricData:
        """计算测试覆盖率"""
        log("计算测试覆盖率...")
    ...
    def calculate_test_success_rate(self) -> MetricData:
        """计算测试成功率"""
        log("计算测试成功率...")
        ...
        try:
            ...
            metric = MetricData(
                ...
                score=score,
                status=status,
                description=f"通过: {passed_tests}, 失败: {failed_tests}, 错误: {error_tests}",
            )

            log(f"  测试成功率: {success_rate:.1f}% (得分: {score:.1f})")
            return metric
    ...
    def calculate_test_execution_time(self) -> MetricData:
        """计算测试执行时间"""
        log("计算测试执行时间...")
        ...
        try:
            ...
            # 分析测试文件数量
            test_files = list(Path("tests/unit").rglob("test_*.py")) + list(
                Path("tests/unit").rglob("*_test.py")
            )
            num_test_files = len(test_files)

            if num_test_files > 0:
                avg_time_per_test = execution_time / num_test_files
            else:
                avg_time_per_test = 0

            target_avg_time = 2.0  # 目标平均执行时间2秒

            # 计算得分（时间越短得分越高）
            if avg_time_per_test <= target_avg_time:
                score = 100.0
            else:
                score = max(
                    0,
                    100
                    - ((avg_time_per_test - target_avg_time) / target_avg_time) * 100,
                )
            ...
            metric = MetricData(
                metric_name="测试执行时间",
                value=avg_time_per_test,
                target_value=target_avg_time,
                weight=0.15,  # 15%权重
                score=score,
                status=status,
                description=f"总执行时间: {execution_time:.1f}秒, 测试文件数: {num_test_files}",
            )

            log(f"  平均执行时间: {avg_time_per_test:.2f}秒/文件 (得分: {score:.1f})")
            return metric
    ...
    def calculate_test_maintainability(self) -> MetricData:
        """计算测试可维护性"""
        log("计算测试可维护性...")
    ...
    def calculate_test_diversity(self) -> MetricData:
        """计算测试多样性"""
        log("计算测试多样性...")
    ...
    def calculate_test_reliability(self) -> MetricData:
        """计算测试可靠性"""
        log("计算测试可靠性...")
        ...
        try:
            ...
            for i in range(3):
                ...
                output = result.stdout
                passed_tests = output.count("PASSED")
                results.append(passed_tests)

            # 计算可靠性（结果一致性）
            if len(results) > 1:
                avg_result = sum(results) / len(results)
                variance = sum((x - avg_result) ** 2 for x in results) / len(results)

                # 计算可靠性得分（方差越小得分越高）
                if variance == 0:
                    reliability_score = 100.0
                else:
                    reliability_score = max(0, 100 - (variance / avg_result) * 100)
            else:
                reliability_score = 0
            ...
            metric = MetricData(
                ...
                status=status,
                description=f"多次运行结果: {results}",
            )

            log(
                f"  测试可靠性得分: {reliability_score:.1f} (得分: {reliability_score:.1f})"
            )
            return metric
    ...
    def _generate_recommendations(
        self, metrics: List[MetricData], weighted_score: float
    ) -> List[str]:
        """生成改进建议"""
        recommendations = []
        ...
        for metric in metrics:
            if metric.score < 70:
                ...
                elif metric.metric_name == "测试执行时间":
                    recommendations.append(
                        f"优化{metric.metric_name}: 当前{metric.value:.2f}秒/文件，目标{metric.target_value}秒"
                    )
                elif metric.metric_name == "测试可维护性":
                    recommendations.append(
                        f"改进{metric.metric_name}: 重构大型测试文件"
                    )
                elif metric.metric_name == "测试多样性":
                    recommendations.append(
                        f"增强{metric.metric_name}: 平衡不同类型测试的比例"
                    )
                elif metric.metric_name == "测试可靠性":
                    recommendations.append(
                        f"提高{metric.metric_name}: 确保测试结果的一致性"
                    )
...
def main():
    ...
    metrics = [
        ...
        quality_metrics.calculate_test_execution_time(),
        quality_metrics.calculate_test_maintainability(),
        quality_metrics.calculate_test_diversity(),
        quality_metrics.calculate_test_reliability(),
    ]
    ...
    result_file = "test_quality_metrics_results.json"
    with open(result_file, "w", encoding="utf-8") as f:
        json.dump(result_data, f, ensure_ascii=False, indent=2)

    log(f"✅ 测试质量指标结果已保存: {result_file}")

    # 创建质量报告
    report_file = "test_quality_metrics_report.md"
    create_quality_report(result_data, report_file)

    log(f"✅ 测试质量指标报告已生成: {report_file}")
...
def create_quality_report(result_data: Dict[str, Any], report_file: str):
    ...
    with open(report_file, "w", encoding="utf-8") as f:
        f.write("# 测试质量指标报告\n\n")
        f.write("## 概述\n")
        f.write("本报告总结了测试质量指标的评估结果和改进建议。\n\n")
