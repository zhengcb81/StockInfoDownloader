...
class TestPerformanceBenchmarkTester:
    ...
    def test_create_test_data(self):
        ...
        expected_dir, actual_dir = self.tester.create_test_data(
            file_count=5, file_size_kb=1, company_count=1, depth=1
        )
    ...
    def test_create_test_data_multiple_companies(self):
        """测试创建多公司测试数据"""
        expected_dir, actual_dir = self.tester.create_test_data(
            file_count=10, file_size_kb=1, company_count=2, depth=1
        )
    ...
    def test_create_test_data_deep_directories(self):
        """测试创建深层目录测试数据"""
        expected_dir, actual_dir = self.tester.create_test_data(
            file_count=3, file_size_kb=1, company_count=1, depth=3
        )
    ...
    def test_benchmark_many_companies(self):
        """测试多公司基准测试"""
        result = self.tester.benchmark_many_companies(
            company_count=3, files_per_company=2
        )
...
class TestPerformanceBenchmarkTesterEdgeCases:
    ...
    def test_very_large_file_size(self):
        ...
        try:
            # 使用较小的文件大小以避免内存问题
            expected_dir, actual_dir = tester.create_test_data(
                file_count=2, file_size_kb=10
            )
            result = tester._run_validation_benchmark(
                "test", expected_dir, actual_dir, 2
            )
