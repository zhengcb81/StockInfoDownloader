...
class PerformanceBenchmarkTester:
    ...
    def create_test_data(
        ...
        files_per_company = (
            file_count // company_count if company_count > 0 else file_count
        )
    ...
    def benchmark_many_companies(
        self, company_count: int = 10, files_per_company: int = 10
    ) -> BenchmarkResult:
        """基准测试：多公司目录"""
        file_count = company_count * files_per_company
        expected_dir, actual_dir = self.create_test_data(
            file_count=file_count, file_size_kb=10, company_count=company_count
        )

        return self._run_validation_benchmark(
            "many_companies", expected_dir, actual_dir, file_count
        )

    def benchmark_deep_directories(
        self, depth: int = 5, file_count: int = 10
    ) -> BenchmarkResult:
        """基准测试：深层目录结构"""
        expected_dir, actual_dir = self.create_test_data(
            file_count=file_count, file_size_kb=10, company_count=1, depth=depth
        )

        return self._run_validation_benchmark(
            "deep_directories", expected_dir, actual_dir, file_count
        )

    def benchmark_comparison_modes(
    ...
    def _run_benchmark(
        self, test_name: str, file_count: int, file_size_kb: int
    ) -> BenchmarkResult:
        """运行基准测试"""
        expected_dir, actual_dir = self.create_test_data(
            file_count=file_count, file_size_kb=file_size_kb
        )
        return self._run_validation_benchmark(
            test_name, expected_dir, actual_dir, file_count
        )

    def _run_validation_benchmark(
        self, test_name: str, expected_dir: Path, actual_dir: Path, file_count: int
    ) -> BenchmarkResult:
        """运行验证基准测试"""
        # 测量执行时间
        start_time = time.time()
    ...
    def run_scalability_test(
        self, scale_factors: List[int] = None
        ...
        for size_kb in size_factors:
            expected_dir, actual_dir = self.create_test_data(
                file_count=10, file_size_kb=size_kb
            )
            result = self._run_validation_benchmark(
                f"file_size_{size_kb}kb", expected_dir, actual_dir, 10
            )
    ...
    def run_stress_test(self, iterations: int = 10) -> Dict[str, Any]:
        ...
        for i in range(iterations):
            expected_dir, actual_dir = self.create_test_data(
                file_count=100, file_size_kb=10
            )
    ...
    def generate_report(self, benchmark_results: Dict[str, Any]) -> str:
        ...
        if "comparison_modes" in benchmark_results:
            ...
            for mode, result in benchmark_results["comparison_modes"].items():
                report_lines.append(
                    f"| {mode} | {result.execution_time_seconds:.4f} | {result.success_rate:.1f} |"
                )
        ...
        if "scalability" in benchmark_results:
            ...
            # 文件数量可扩展性
            file_count_results = benchmark_results["scalability"].get(
                "file_count_scalability", []
            )
            if file_count_results:
                ...
                for result in file_count_results:
                    report_lines.append(
                        f"| {result.file_count} | {result.execution_time_seconds:.4f} |"
                    )

            # 文件大小可扩展性
            file_size_results = benchmark_results["scalability"].get(
                "file_size_scalability", []
            )
            if file_size_results:
                ...
                for result in file_size_results:
                    size_kb = (
                        result.total_size_bytes / (1024 * result.file_count)
                        if result.file_count > 0
                        else 0
                    )
                    report_lines.append(
                        f"| {size_kb:.0f} | {result.execution_time_seconds:.4f} |"
                    )
        ...
        if "stress_test" in benchmark_results:
            ...
            report_lines.append(
                f"- 最小执行时间: {stress_results['min_execution_time']:.4f} 秒"
            )
            report_lines.append(
                f"- 最大执行时间: {stress_results['max_execution_time']:.4f} 秒"
            )
            report_lines.append(
                f"- 执行时间标准差: {stress_results['std_execution_time']:.4f} 秒"
            )
            report_lines.append(
                f"- 平均成功率: {stress_results['avg_success_rate']:.1f}%"
            )
