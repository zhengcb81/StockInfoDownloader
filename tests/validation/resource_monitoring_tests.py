...
class ResourceMonitor:
    ...
    def stop_monitoring(self) -> ResourceUsage:
        ...
        execution_time = self.monitoring_data[-1]['timestamp'] - self.monitoring_data[0]['timestamp']
...
class ResourceMonitoringTester:
    ...
    def create_test_data(self, file_count: int, file_size_kb: int = 10) -> Tuple[Path, Path]:
        """创建测试数据"""
    ...
    def monitor_stress_scenario(self, iterations: int = 10) -> Dict[str, Any]:
        ...
        for i in range(iterations):
            expected_dir, actual_dir = self.create_test_data(file_count=50, file_size_kb=10)
    ...
    def _detect_memory_leak(self, memory_usage: List[float]) -> bool:
        ...
        first_half_avg = sum(memory_usage[:len(memory_usage)//2]) / (len(memory_usage)//2)
        second_half_avg = sum(memory_usage[len(memory_usage)//2:]) / (len(memory_usage) - len(memory_usage)//2)
        
        # 如果后半段平均内存使用比前半段高10%，认为可能有内存泄漏
        return second_half_avg > first_half_avg * 1.1
    
    def _run_resource_monitoring(self, test_name: str, file_count: int) -> ResourceMonitoringResult:
        """运行资源监控测试"""
    ...
    def generate_monitoring_report(self, monitoring_results: Dict[str, Any]) -> str:
        ...
        report_lines.append("| 测试名称 | 文件数量 | 内存使用 (MB) | CPU使用 (%) | 文件句柄 | 执行时间 (秒) | 峰值内存 (MB) |")
        report_lines.append("|---------|---------|--------------|------------|----------|--------------|--------------|")
        ...
        if "stress_test" in monitoring_results:
            ...
            report_lines.append(f"- 平均内存使用: {stress_results['avg_memory_usage_mb']:.2f} MB")
            report_lines.append(f"- 最大内存使用: {stress_results['max_memory_usage_mb']:.2f} MB")
            report_lines.append(f"- 平均CPU使用: {stress_results['avg_cpu_percent']:.1f}%")
            report_lines.append(f"- 最大CPU使用: {stress_results['max_cpu_percent']:.1f}%")
            report_lines.append(f"- 平均执行时间: {stress_results['avg_execution_time']:.4f} 秒")
            report_lines.append(f"- 内存泄漏检测: {'是' if stress_results['memory_leak_detected'] else '否'}")
