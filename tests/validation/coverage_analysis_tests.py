...
class TestAnalyzer:
    ...
    def extract_test_targets(self, test_file_path: Path) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            # 匹配测试类：Test开头的类
            # 支持: class TestFoo:, class TestFoo(TestCase):, class TestFoo:
...
class CoverageAnalysisTool:
    ...
    def _match_test_targets(
                ...
                for file_path, coverage in file_coverage.items():
                    ...
                    for class_info in coverage.classes:
                        for method in class_info.methods:
                            if method.function_name == function_name.replace(
                                "test_", ""
                            ):
        ...
        for file_path, coverage in file_coverage.items():
            ...
            # Count class as tested if explicitly tested OR if any method is tested
            tested_classes = 0
            for c in coverage.classes:
                methods_tested = any(m.is_tested for m in c.methods)
                if c.is_tested or methods_tested:
                    c.is_tested = True  # Ensure it's marked
                    tested_classes += 1
            ...
            coverage_percentage = (
                (tested_entities / total_entities * 100) if total_entities > 0 else 0.0
            )
    ...
    def _calculate_overall_coverage(
        ...
        tested_files = sum(
            1 for coverage in file_coverage.values() if coverage.coverage_percentage > 0
        )

        total_functions = sum(
            coverage.total_functions for coverage in file_coverage.values()
        )
        tested_functions = sum(
            coverage.tested_functions for coverage in file_coverage.values()
        )

        total_classes = sum(
            coverage.total_classes for coverage in file_coverage.values()
        )
        tested_classes = sum(
            coverage.tested_classes for coverage in file_coverage.values()
        )
    ...
    def generate_coverage_report(self, analysis_result: CoverageAnalysisResult) -> str:
        ...
        for file_path, coverage in analysis_result.file_coverage.items():
            func_coverage = (
                (coverage.tested_functions / coverage.total_functions * 100)
                if coverage.total_functions > 0
                else 0.0
            )
            class_coverage = (
                (coverage.tested_classes / coverage.total_classes * 100)
        ...
        if analysis_result.high_complexity_untested:
            ...
            for function in analysis_result.high_complexity_untested:
                report_lines.append(
                    f"| {function.function_name} | {function.file_path} | {function.complexity} |"
                )
        ...
        if analysis_result.untested_functions:
            ...
            for function in analysis_result.untested_functions[:20]:  # 只显示前20个
                report_lines.append(
                    f"| {function.function_name} | {function.file_path} |"
                )
        ...
        if analysis_result.untested_classes:
            ...
            for class_info in analysis_result.untested_classes:
                report_lines.append(
                    f"| {class_info.class_name} | {class_info.file_path} |"
                )
...
CoverageAnalyzer = CoverageAnalysisTool
