...
def test_cleaner_basic_functionality():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        ), f"应该只保留1个目录，实际保留{status['directories']}个"
...
def test_cleaner_with_real_config():
    ...
    backup_dir = (
        save_dir.parent
        / f"{save_dir.name}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    try:
        ...
        # 创建测试文件（使用真实的股票代码名称）
        preserve_cases = [
            case for case in config["test_cases"] if not case.get("delete_later", True)
        ]
        delete_cases = [
            case for case in config["test_cases"] if case.get("delete_later", True)
        ]
        ...
        assert (
            status_after["directories"] == expected_preserved_dirs
        ), f"期望保留 {expected_preserved_dirs} 个目录，实际保留 {status_after['directories']} 个"
        log("✓ 真实配置测试通过")
...
def test_cleaner_dry_run():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            status_after_dry["directories"] == 2 and result_dry["dry_run"]
        ), "模拟运行测试失败"
        log("✓ 模拟运行测试通过")
        return True
