...
class TestKeywordMatcher:
    ...
    def test_any_mode_matching(self):
        """测试any模式的匹配"""
        config = KeywordConfig(allowed_keywords=["投资者关系", "调研"], mode="any")
    ...
    def test_all_mode_matching(self):
        """测试all模式的匹配"""
        config = KeywordConfig(allowed_keywords=["投资者关系", "调研"], mode="all")
        matcher = KeywordMatcher(config)
    ...
    def test_regex_mode_matching(self):
        ...
        config = KeywordConfig(
            allowed_keywords=[r"投资者.*调研", r"关系.*活动"], mode="regex"
        )
        matcher = KeywordMatcher(config)
    ...
    def test_exclude_keywords(self):
        """测试排除关键词"""
        config = KeywordConfig(
            allowed_keywords=["投资者关系"], exclude_keywords=["更正"]
        )
        matcher = KeywordMatcher(config)
    ...
    def test_case_insensitive_matching(self):
        """测试大小写不敏感匹配"""
        config = KeywordConfig(allowed_keywords=["Investor"], mode="any")
        matcher = KeywordMatcher(config)
    ...
    def test_multi_field_matching(self):
        """测试多字段匹配"""
        config = KeywordConfig(
            allowed_keywords=["调研"], fields=["title", "content", "date"]
        )
        matcher = KeywordMatcher(config)
    ...
    def test_invalid_regex_handling(self):
        """测试无效正则表达式的处理"""
        config = KeywordConfig(allowed_keywords=["[invalid"], mode="regex")
        matcher = KeywordMatcher(config)
    ...
    def test_validate_keywords(self):
        """测试关键词验证"""
        # 有效的正则表达式
        config = KeywordConfig(allowed_keywords=[r"\d+", r"[a-z]+"], mode="regex")
        matcher = KeywordMatcher(config)
        assert matcher.validate_keywords() is True

        # 无效的正则表达式
        config = KeywordConfig(allowed_keywords=[r"[invalid"], mode="regex")
        matcher = KeywordMatcher(config)
