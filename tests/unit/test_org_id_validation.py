...
from src.tools.legacy.validate_org_id import (
    validate_and_refresh_org_id,
    validate_org_id_url,
)
...
class TestOrgIdValidation(unittest.TestCase):
    ...
    def test_validate_and_refresh_org_id_valid(self):
        """测试验证并刷新有效org ID"""
        with patch(
            "src.tools.legacy.validate_org_id.validate_org_id_url"
        ) as mock_validate, patch(
            "src.tools.legacy.validate_org_id.invalidate_cache"
        ) as mock_invalidate:
    ...
    def test_validate_all_cached_org_ids(self):
        """测试批量验证所有缓存org ID"""
        with patch(
            "src.tools.legacy.validate_all_cached.validate_org_id_url"
        ) as mock_validate, patch(
            "src.tools.legacy.validate_all_cached.invalidate_cache"
        ) as mock_invalidate:

            mock_validate.side_effect = [True, True]  # 两个都有效

            report = validate_all_cached_org_ids(
                self.test_mapping_file, fix_invalid=False
            )
    ...
    def test_validate_all_cached_org_ids_with_fix(self):
        ...
        with patch(
            ...
            report = validate_all_cached_org_ids(
                self.test_mapping_file, fix_invalid=True
            )
...
class TestOrgIdRegression(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.historical_data = {
            ...
            "known_invalid": {"300470": "9900008470", "000001": "invalid123"},
        }
        ...
        for config in configs:
            with self.subTest(config=config):
                try:
                    ...
                    with patch(
                        "src.tools.legacy.validate_org_id.ConfigManager"
                    ) as mock_config:
...
class TestOrgIdIntegration(unittest.TestCase):
    ...
    def test_end_to_end_validation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 创建测试映射
            test_data = {"300470": {"orgId": "9900023856", "name": "中密控股"}}
