...
class TestDataValidator:
    ...
    def test_validate_url_require_https(self):
        ...
        is_valid, result = self.validator.validate(
            "http://example.com", "url", require_https=True
        )
        assert is_valid is False

        is_valid, result = self.validator.validate(
            "https://example.com", "url", require_https=True
        )
        assert is_valid is True
    ...
    def test_validate_date_future_not_allowed(self):
        ...
        is_valid, result = self.validator.validate(
            future_date, "date", allow_future=False
        )
        assert is_valid is False
    ...
    def test_validate_numeric_range_valid(self):
        ...
        # 测试整数
        is_valid, result = self.validator.validate(
            50, "numeric_range", min_val=0, max_val=100
        )
        assert is_valid is True
        assert result == 50.0
        ...
        # 测试浮点数
        is_valid, result = self.validator.validate(
            3.14, "numeric_range", min_val=0, max_val=10
        )
        assert is_valid is True
        assert result == 3.14
    ...
    def test_validate_numeric_range_invalid(self):
        ...
        # 超出范围
        is_valid, result = self.validator.validate(
            150, "numeric_range", min_val=0, max_val=100
        )
        assert is_valid is False
    ...
    def test_validate_string_length_valid(self):
        """测试有效字符串长度验证"""
        is_valid, result = self.validator.validate(
            "hello", "string_length", min_length=3, max_length=10
        )
        assert is_valid is True
        assert result == "hello"
    ...
    def test_validate_string_length_invalid(self):
        ...
        # 太长
        is_valid, result = self.validator.validate(
            "this is too long", "string_length", max_length=10
        )
        assert is_valid is False
    ...
    def test_validate_enum_value_valid(self):
        ...
        for value in allowed_values:
            is_valid, result = self.validator.validate(
                value, "enum_value", allowed_values=allowed_values
            )
            assert is_valid is True
    ...
    def test_validate_enum_value_case_insensitive(self):
        ...
        is_valid, result = self.validator.validate(
            "option1", "enum_value", allowed_values=allowed_values, case_sensitive=False
        )
        assert is_valid is True
    ...
    def test_validate_enum_value_invalid(self):
        ...
        allowed_values = ["option1", "option2"]
        is_valid, result = self.validator.validate(
            "invalid", "enum_value", allowed_values=allowed_values
        )
        assert is_valid is False
    ...
    def test_validate_batch_with_errors(self):
        """测试批量验证包含错误"""
        data = {"stock_code": "invalid", "company_name": "", "email": "invalid_email"}
