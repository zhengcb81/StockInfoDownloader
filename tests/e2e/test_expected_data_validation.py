...
class TestExpectedDataValidationE2E:
    ...
    def test_expected_vs_actual_validation_strict_mode(self):
        """测试严格模式下的期待数据与实际数据验证"""
        # 模拟下载过程 - 只下载一个股票
        downloaded_files = self._simulate_download(test_stocks[0]["code"], self.save_dir)
        ...
        for dir_result in result.directory_results:
            if dir_result.actual_exists and dir_result.directory_path.endswith(test_stocks[0]["name"]):
    ...
    def test_expected_vs_actual_validation_lenient_mode(self):
        ...
        # 模拟下载过程
        downloaded_files = self._simulate_download(test_stocks[1]["code"], self.save_dir)
        ...
        downloaded_stock_result = None
        for dir_result in result.directory_results:
            if dir_result.actual_exists and dir_result.directory_path.endswith(test_stocks[1]["name"]):
                downloaded_stock_result = dir_result
                break
    ...
    def test_single_test_case_validation(self):
        """测试单个测试用例的验证"""
        # 模拟下载过程
        downloaded_files = self._simulate_download(test_stocks[2]["code"], self.save_dir)
    ...
    def test_validation_with_missing_files(self):
        ...
        expected_file = os.path.join(self.expected_data_dir, stock_info["name"], filename)
