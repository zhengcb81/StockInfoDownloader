...
class BaseDownloader(IDownloader):
    ...
    def _execute_download_with_retry(self, request: DownloadRequest) -> DownloadResult:
        ...
        for attempt in range(retry_count + 1):
            try:
                ...
                self.logger.warning(
                    f"Download attempt {attempt + 1}/{retry_count + 1} failed: {e}"
                )

                # Check if should retry
                should_retry = attempt < retry_count and (
                    not self.anti_crawler_strategy
                    or self.anti_crawler_strategy.should_retry(e)
                )
...
class BaseAntiCrawlerStrategy(IAntiCrawlerStrategy):
    ...
    def _should_delay_request(self) -> bool:
        ...
        if (
            self.session_requests >= self._get_session_limit()
