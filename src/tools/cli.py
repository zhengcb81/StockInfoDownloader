...
from src.tools.stock_tools import StockMappingTool, StockValidationTool, StockDebugTool
...
def main():
    parser = argparse.ArgumentParser(description="StockInfoDevKit CLI - Unified Developer Tools")
    subparsers = parser.add_subparsers(dest='tool', help='Tool to use')

    # Mapping Tool
    mapping_parser = subparsers.add_parser('mapping', help='Manage stock code to OrgID mapping')
    mapping_parser.add_argument('action', choices=['get', 'update', 'list'], help='Action to perform')
    mapping_parser.add_argument('--stock-code', help='Stock code')
    mapping_parser.add_argument('--headless', action='store_true', default=True, help='Run in headless mode')

    # Validation Tool
    validation_parser = subparsers.add_parser('validation', help='Validate OrgIDs and cache consistency')
    validation_parser.add_argument('--stock-code', help='Stock code to validate')
    validation_parser.add_argument('--all', action='store_true', help='Validate all cached entries')

    # Debug Tool
    debug_parser = subparsers.add_parser('debug', help='Debug system state and flow')
    debug_parser.add_argument('--category', choices=['flow', 'structure'], default='flow', help='Debug category')
