...
try:
    from .orgid_utils import _load_mapping, get_org_id_by_code
except ImportError:
    from orgid_utils import _load_mapping, get_org_id_by_code
...
def validate_org_id_url(
    stock_code: str, org_id: str, config: Dict[str, Any] = None
) -> bool:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if is_valid:
                ...
                logger.warning(
                    f"验证失败: {stock_code} 的 org ID {org_id} 无效 (状态码: {response.status_code})"
                )
                return False
...
def invalidate_cache(stock_code: str, mapping_file: str = None) -> Optional[str]:
    ...
    try:
        ...
        new_org_id = get_org_id_by_code(
            stock_code, force_run=True, mapping_file=mapping_file
        )
...
def validate_and_refresh_org_id(
    stock_code: str, mapping_file: str = None
) -> Optional[str]:
    """
    ...
    try:
        ...
        if is_valid:
            ...
            logger.warning(
                f"⚠️  {stock_code} 的 org ID {current_org_id} 无效，重新获取..."
            )
            return invalidate_cache(stock_code, mapping_file)
        else:
            logger.warning(
                f"⚠️  {stock_code} 的 org ID {current_org_id} 无效但auto_invalidate=false"
            )
            return current_org_id
