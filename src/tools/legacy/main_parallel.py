...
from main import UnifiedRunner, get_real_stock_name
...
class CompanyConfig:
    ...
    def __init__(self, stock_code: str, company_name: str = "", enabled: bool = True, **kwargs):
...
class MultiCompanyDownloader:
    ...
    def download_company(self, company_config: CompanyConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            files_count = len(res) if isinstance(res, list) else (res.total_files if hasattr(res, 'total_files') else 0)
    ...
    def download_companies_sequential(self, configs: List[CompanyConfig]) -> List[Dict[str, Any]]:
    ...
    def download_companies_parallel(self, configs: List[CompanyConfig]) -> List[Dict[str, Any]]:
