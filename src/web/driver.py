...
class WebDriverManager:
    ...
    def __init__(
        ...
        self.headless = (
            ...
            else self.config_manager.get("webdriver.headless", True)
        )
        self.window_size = (
            window_size
            if window_size is not None
            else self.config_manager.get("webdriver.window_size", "1920,1080")
        )
        self.page_load_timeout = (
            page_load_timeout
            if page_load_timeout is not None
            else self.config_manager.get("timeout.page_load", 8)
        )
        self.implicit_wait = (
            implicit_wait
        ...
        self.max_downloads_per_session = (
            ...
            if max_downloads_per_session is not None
            else self.config_manager.get("download.max_downloads_per_session", 10)
        )
        self._user_agents = self.config_manager.get(
    ...
    def restart_driver(
        ...
        if enhanced_retry:
            ...
            for attempt in range(max_restart_attempts):
                try:
                    logger.info(
                        f"尝试重启WebDriver (第 {attempt + 1}/{max_restart_attempts} 次)..."
                    )
    ...
    def wait_for_element(
        self, by: str, value: str, timeout: int = 10, condition: str = "presence"
    ) -> bool:
        """
        等待元素出现
