...
class AntiCrawlerStrategy:
    ...
    def simulate_complex_browsing(self, driver):
        ...
        try:
            ...
            selected_behaviors = random.sample(
                behaviors, min(num_behaviors, len(behaviors))
            )
    ...
    def _simulate_scrolling(self, driver):
        ...
        try:
            ...
            selected_scrolls = random.sample(
                scroll_positions, min(num_scrolls, len(scroll_positions))
            )
    ...
    def set_session_parameters(
        self, min_delay: float, max_delay: float, max_downloads: int
    ):
        """
        设置会话参数
    ...
    def check_session_limit(self, current_downloads: int) -> bool:
        ...
        if current_downloads >= self.max_session_downloads:
            logger.info(
                f"会话下载限制达到: {current_downloads}/{self.max_session_downloads}"
            )
            return True
        return False
    ...
    def set_session_parameters(
        self, min_delay: float = 1.0, max_delay: float = 5.0, max_downloads: int = 5
    ) -> None:
        """
        设置会话参数
        ...
        self.max_session_downloads = max_downloads
        logger.info(
            f"会话参数已设置: 延迟{min_delay}-{max_delay}秒, 最大下载{max_downloads}次"
        )
