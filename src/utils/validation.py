...
from .security import sanitize_filename, validate_org_id, validate_stock_code
...
class DataValidator:
    ...
    def _validate_stock_code_enhanced(
        self, stock_code: str, allow_empty: bool = False, check_existence: bool = False
    ) -> str:
    ...
    def _validate_org_id_enhanced(
        self, org_id: str, allow_empty: bool = False, check_format: bool = True
    ) -> str:
        ...
        if check_format and not validate_org_id(org_id):
            raise ValidationError(
                "Invalid org ID format, must be 10 digits starting with 99"
            )
    ...
    def _validate_company_name(
        ...
        if len(name) < min_length:
            raise ValidationError(
                f"Company name length cannot be less than {min_length}"
            )
        ...
        for char in forbidden_chars:
            if char in name_lower:
                raise ValidationError(
                    f"Company name contains forbidden character: {char}"
                )
            ...
            try:
                if format_pattern:
                    parsed_date = datetime.strptime(
                        date_value.strip(), format_pattern
                    ).date()
                else:
                    # Try common formats
    ...
    def _validate_phone(
        self, phone: str, allow_empty: bool = False, country_code: str = "CN"
    ) -> str:
    ...
    def _validate_file_path(
        ...
        if allowed_extensions:
            ...
            if path_obj.suffix.lower() not in [
                ...
                raise ValidationError(
                    f"File extension not allowed: {allowed_extensions}"
                )
    ...
    def validate_batch(
        self, data_dict: Dict[str, Any], validation_rules: Dict[str, Dict]
    ) -> Dict[str, Any]:
        """
...
def validate_batch_data(
    data_dict: Dict[str, Any], validation_rules: Dict[str, Dict]
) -> Dict[str, Any]:
