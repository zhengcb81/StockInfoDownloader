...
def main():
    ...
    # 创建下载器 (使用统一下载器)
    try:
        downloader = downloader_factory.create_downloader("unified", save_dir=save_dir)
    except Exception as e:
    ...
    if pages:
        ...
        total_success = downloader.download_activity_records(
            stock_code, headless=headless, max_retries=max_retries
        )
