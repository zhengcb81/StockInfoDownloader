...
def search_target_document():
    """全面搜索目标文档"""
    logger.info("开始全面搜索目标文档...")
    ...
    try:
        with driver_manager as driver:
            ...
            config = KeywordConfig(allowed_keywords=[target_keyword])
            ...
            for page_num in range(1, max_pages_to_search + 1):
                ...
                for element in all_text:
                    try:
                        ...
                        if text and len(text) > 20:  # 只检查较长的文本
                            if keyword_matcher.matches(text):
                ...
                for i, (element, text) in enumerate(matching_elements):
                    logger.info(f"匹配元素 {i+1}: '{text[:80]}...'")
                        ...
                        try:
                            ...
                            for link in links:
                                ...
                                if "/new/disclosure/detail" in href:
                                    logger.info(f"  -> 详情页链接: {href[:100]}...")
                                    link_found = True
                                    
                                    # 检查是否是目标文档
                                    if "2023" in href and "0131" in href:  # 1月31日
                                        logger.info(f"  -> 可能是目标文档!")
                                        found = True
                                    break
                            if link_found:
                                break
                ...
                for i, element in enumerate(jan_elements[:10]):
                    try:
                        ...
                        if text and len(text) > 10:
                            logger.info(f"2023年1月元素 {i+1}: '{text[:60]}...'")
                    except:
                        continue
